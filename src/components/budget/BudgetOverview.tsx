...
const BudgetOverview: React.FC = () => {
  ...
  const [editValue, setEditValue] = useState(budget.amount.toString());
  ...
  return (
    <div className="bg-white/[0.02] backdrop-blur-xl rounded-2xl p-6 border border-white/5">
      ...
      {isEditingBudget ? (
        ...
        <div className="mb-6">
          <div className="flex items-baseline gap-2 mb-1">
            <span className="text-3xl font-bold">{formatINR(budget.amount, false)}</span>
      ...
      {/* Alert Banner */}
      {budget.alertMessage && budget.alertLevel !== 'none' && (
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="p-4 bg-white/[0.02] rounded-xl border border-white/5">
          ...
          <p className="text-xl font-bold text-red-400">
            {formatINR(budget.totalExpense, false)}
          </p>
        </div>
        <div className="p-4 bg-white/[0.02] rounded-xl border border-white/5">
          <p className="text-xs text-gray-400 mb-1">Remaining</p>
          <p className={`text-xl font-bold ${budget.remainingBudget < 0 ? 'text-red-400' : 'text-green-400'}`}>
            {formatINR(Math.abs(budget.remainingBudget), false)}
            {budget.remainingBudget < 0 && <span className="text-sm ml-1">(over)</span>}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        {budget.usagePercentage > 100 && (
          <p className="text-xs text-red-400 text-right">
            {(budget.usagePercentage - 100).toFixed(1)}% over budget
          </p>
        )}
      </div>
        ...
        <div className="mt-4 pt-4 border-t border-white/5">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-semibold text-blue-400">
              {formatINR(budget.totalIncome, false)}
            </span>
          </div>
        </div>
      )}
